...
const context={document:{getElementById:el},Option:function(){},URLSearchParams,AbortSignal,
 fetch:async(url,options)=>{
  ...
  return{ok:true,json:async()=>url==='/api/state'?{token:'lamp-token',effects:Array(37).fill('Effect'),mode:30,colors:Array.from({length:37},()=>[1,255,60,110]),effectOptions:Array.from({length:37},()=>[50,100,1,35,160,255])}:{phase:0,wifi:false}};
 }};
...
(async()=>{
 ...
 el('mode').value='37';el('mode').onchange();assert.equal(el('speed').value,50);
