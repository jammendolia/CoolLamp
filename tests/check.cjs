...
assert.equal(modes.length, 37);
assert.deepEqual(modes.map(m=>Number(m[2])), Array.from({length:37},(_,i)=>i+1));
...
console.log('PASS: page JavaScript parses; 37 modes have unique selectable IDs; split/rain mappings cover lengths 1–1024; controls contain no blocking serial writes.');
