...
assert.deepEqual(modes.map(m=>Number(m[2])), Array.from({length:29},(_,i)=>i+1));
...
console.log('PASS: page JavaScript parses; 29 modes have unique selectable IDs; split/rain mappings cover lengths 1–1024; controls contain no blocking serial writes.');
