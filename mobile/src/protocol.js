...
const operations = { power: 1, brightness: 2, effect: 3, saveDefaults: 4, refresh: 5, color: 6, resetColor: 7, checkFirmware: 8, installFirmware: 9, autoUpdate: 10, effectOptions: 11, enableEffects: 12 };
export function encodeCommand(id, operation, value = 0) {
  ...
  if (operation === 'effectOptions') {
    ...
    if (!Number.isInteger(id) || id < 1 || id > 255 || !v ||
        ...
        v.mode < 1 || v.mode > effects.length || v.speed < 1 || v.speed > 100 ||
  ...
  const valid = op === 1 || op === 10 ? value >= 0 && value <= 1 : op === 2 ? value >= 1 && value <= 255 :
    op === 3 || op === 7 ? value >= 1 && value <= effects.length : op === 12 ? value === 1 : value === 0;
...
export function decodeState(data) {
  ...
  if (data.getUint8(5) > 1 || state.mode < 1 || state.mode > state.effectCount || state.brightness < 1 || ![28,29,37].includes(state.effectCount)) {
...
export function decodeEffectOptions(data) {
  ...
  encodeCommand(1,'effectOptions',value);
  return value;
}
