...
int UpdateHttp::receive(void* output,size_t size){
  ...
  return esp_tls_conn_read(tls,output,size);
}
bool UpdateHttp::send(const char* text){
  size_t left=strlen(text);
  while(left){const int n=esp_tls_conn_write(tls,text,left);if(n<=0)return false;text+=n;left-=n;}
  return true;
}
bool UpdateHttp::open(String url,int& code,int64_t& length){
  close();done=false;started=millis();
  for(unsigned redirects=0;redirects<6;++redirects){
    ...
    if(esp_tls_conn_new_sync(host.c_str(),host.length(),443,&config,tls)!=1){close();return false;}
    ...
    while(!headers.complete()){
      ...
      if(!headers.feed(char(buffer[cursor++]))){close();return false;}
    }
