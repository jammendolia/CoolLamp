...
const stageRoot = path.join(root, '.build/sketch');
...
for (const entry of fs.readdirSync(sketch, { withFileTypes: true })) {
  if (entry.isFile() && /\.(ino|cpp|h)$/.test(entry.name) && !fs.existsSync(path.join(root, entry.name))) fs.unlinkSync(path.join(sketch, entry.name));
}
for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
  if (entry.isFile() && /\.(ino|cpp|h)$/.test(entry.name)) fs.copyFileSync(path.join(root, entry.name), path.join(sketch, entry.name));
}
if (!fs.existsSync(path.join(sketch, 'LampSecrets.h'))) throw new Error('Run node tools/setup-secrets.cjs before building.');
const flavor = wifiOnly ? 'wifi' : 'ble';
const board = 'esp32:esp32:esp32c3:CDCOnBoot=cdc' + (wifiOnly ? '' : ',PartitionScheme=no_fs');
