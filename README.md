...
5. Preview effects/brightness, or save LED count, brightness, startup effect, LED current limit, home Wi-Fi, and a new access password. Saving restarts the lamp.
...
After cloning, run `node tools/setup-secrets.cjs` once to create a private, unique initial password header. It will not overwrite an existing header. Node.js is needed for this helper and the tests.
